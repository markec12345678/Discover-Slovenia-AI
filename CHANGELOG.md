...
- **Samo-vzdržni sitemap števec**: `getEnSitemapUrlCount()` trdo kodiranih `11` zamenjalo `EN_STATIC_ROUTES.size` (izvoz iz routing.ts) — sitemap test je padel natanko na tej številki (364→365 EN URL-jev, skupaj 737) in zdaj ne more več pasti: dodajanje poti na whitelisto samodejno posodobi števec. Prag smoke testa (≥300) ni prizadet.
