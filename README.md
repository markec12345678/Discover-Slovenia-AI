...
- **DB migracije na produkcijo:** `./scripts/ops/migrate-deploy.sh "<neon-url>"` (1.36.1 — varni `prisma migrate deploy` tudi iz klona z lokalno sqlite shemo; `--status` = read-only vpogled v čakajoče migracije; trenutno čaka `20260916100000_restrict_money_fks` iz 1.36.0)
