...
**Status faz:** P0 ✅ → P1 ✅ → P2 ✅ → P3 ✅ (varnostni auditi) → P4 ✅ (pilotni polish) → P5 ✅ (priprava deploya) → P6 ✅ (sinhronizacija dokumentov) → P7 ✅ (varnostni audit + popravki P0–P2) → P8 ✅ (responsive 390 px + atomarna booking deduplikacija) → P9 ✅ (code freeze + deploy runbook/smoke orodja) → R2/R3 auditi ✅ → FW1 ✅ (kritični audit popravki) → FW2 ✅ (UX quick wins — Mindtrip Tier 1) → FW3 ✅ (AI-first hierarhija UX refaktor) → **MONET ✅ (monetizacijska mreža — 10 partnerjev fail-closed, affiliate + GEO/SEO paket + mobilni UX pass)** → TASK 42–51 ✅ (Supply Map + supply-aware AI/FIXED + geo koherenca — 947/947) → TASK 52 ✅ (produkcijska matrika 16 providerjev — 1004/1004) → **TASK 53 ✅ (ALL PROVIDERS READY WITHOUT API KEYS — 10 adapterjev, 1196/1196 testov, lint 0, tsc 0; 3 LIVE, 8 CODE READY z živo preverjenimi pogodbami, 0 fake inventarja; prihodnja aktivacija = samo env vnos — glej [TASK-53 matriko](docs/TASK-53-ALL-PROVIDERS-READY.md))** → **TASK 54 ✅ (GitHub-first revizija aktivacije — vrata VSEH gated providerjev živo preverjena 401/403 s pogodbenimi glavami; 3 LIVE, 27 env imen MISSING iskreno; `.env.example` usklajen z kodo; aktivacija BREZ spremembe kode — glej [TASK-54 revizijo](docs/TASK-54-LIVE-PROVIDER-ACTIVATION.md))** → **TASK 55 ✅ (neodvisna revizija TASK 54 — vse materialne trditve potrjene s kodo + živimi preverbami; 0 P0/0 P1)** → **TASK 56 ✅ (P2 data integrity hardening — /go canonical membership 404 za fabricated ID-je, save-meja kanonska revalidacija (ista veriga kot refine echo), refine notes = dokumentirani FOLLOW-UP; +18 testov 1214/1214 — glej [TASK-56](docs/TASK-56-P2-HARDENING.md))** → **TASK 58 ✅ (FULL PROVIDER JOURNEY — orkestrator celotnega potovanja Brnik→Maribor prek obstoječih ponudnikov: /potovanje UI, KT iskanje po ruti, bencin (OSM), tokovi rezervacije z EXTERNAL≠CONFIRMED, skupna cena z ločbo od-cene/neznano, JourneyBooking model potrditev; +45 testov 1259/1259 — glej [TASK-58](docs/TASK-58-FULL-PROVIDER-JOURNEY.md) + [revizijo zmožnosti](docs/TASK-58-JOURNEY-AUDIT.md))** → **TASK 58 §20–§32 ✓ (MY TRIP časovnica + potrditveni dokument brez izmišljenih številk, provider-agnostic preoblikovanje, izolacija odpovedi, observability, +33 sprejemnih testov 1292/1292 — glej [MY TRIP](docs/TASK-58-MY-TRIP-ACCEPTANCE.md))** — pilot-ready → **TASK 61 ✅ (FSQ OS PLACES AKTIVACIJA — LOCALNA odprta množica nameščena: SI+HR+ME+AL, 125.446 krajev (si 18k/hr 88k/me 10k/al 8,8k, 36,6 MB git baseline), snapshot 2025-02-06 iz primarne distribucije fused.io (source.coop, DuckDB httpfs range-pushdown — brez gated HuggingFace); dodelitev držav po LASTNI KODI vira (0 IT/GR/BA krajev), hierarhični label-poti vira (segment-match + terminal subkategorija), potovalno-relevanten kategorjski obseg, bencin (petrol) novi tip fsq plasti (21 postaj v LJU — pokriva mrtvi OSM sloj v peskovniku); 4. provider PRODUCTION_ACTIVE (LIVE); osvežitev: bun run fsq:ingest; 1295/1295 testov)** → **TASK 62 ✅ (REGIONALNA POKRITOST POTOVANJ — destinacijski register SI+HR+ME+AL: 22 SI + 16 novih destinacij zahodnega Balkana (Zagreb/Plitvice/Rijeka/Pula/Zadar/Split/Hvar/Dubrovnik + Kotor/Budva/Podgorica/Durmitor + Tirana/Berat/Gjirokastër/Sarandë), country polje + 11 regij + EN overlay + VLM-auditirane AI slike; /potovanje izbirnik optgroup po državah z signalom cene/ocene (€€ · ★4,8); /destinacije filter Država (6 filtrov); izvoriščni fallback KT geo (Brnik tudi za Dubrovnik); AI kontekst 38 destinacij + pravilo obsega držav (privzeto SI, regionalno SAMO izrecno); fallback bazen + AI supply bbox ostajata slovenska (geo-koherenca); 1314/1314 testov)**; produkcijo preveri dinamično (smoke skripta, točka 8)
...
> - **Koda:** `main` = glej git log (1.48.3 — PRAZNI AI DNEVI → REZERVA: živ dokaz na Vercelu (source "ai" + days [] + total_budget 0 — :free model je vrnil popoln JSON z neveljavno strukturo dni, sanitize je legitimno porezal vse, razlaga/priporočila pa preživeli → uporabniku PRAZEN načrt z AI badgeom); stražar v rundi po sanitize vrže na OBSTOJEČO catch→fallback pot (nič nove logike, samo klasifikacija praznega izhoda kot neuspeha generacije; v rundi in ne v sanitizeItinerary, ker sanitize teče tudi na save meji klientovih načrtov); tsc 0, eslint 0, 145/145; pred tem 1.48.2 — PER-KLIC AI PRORAČUN (free tier realnost zlate poti): direktne meritve OpenRouter z itinerary-velikim JSON promptom (enak ključ + model kot produkcija, 2026-09-17): 60 s / 61 s / 79 s — 3/3 vzorci NA ali ČEZ privzeti 60-s budilnik `OPENROUTER_TIMEOUT_MS` → približno vsak drugi klic načrtovalnika je budilnik rezal v deterministični fallback, uporabnik pa čakal prav toliko časa (fallback pride šele po koncu OR poskusov); `AICompletionOptions.timeoutMs` (SDK v6 per-request `timeout`, singleton odjemalec nedotaknjen) + 2 varovali vezane najslabše časovnice: ob izrecnem proračunu SDK auto-retry IZKLOPLJEN (`maxRetries: 0` — notranji fallback model je že naša retry plast) in ob `APIConnectionTimeoutError` rezervni model PRESKOČEN (:free čakalna vrsta je skupna vsem modelom — sicer do 4× čas), hitre napake (429/5xx) notranji fallback poskusi še vedno; itinerary podaljša proračun na 120 s (UX: po ~isti potrpežljivosti PRAVI AI načrt namesto rezerve), klepet ostane na 60 s (osveščena odločitev — hitrejši pošten fallback za hitre klice); pripadajoče odkritje: `nex-agi :free` ob zelo velikih promptih včasih vrne 200 s PRAZNO vsebino (2/2 lokalno danes, manjši direktni prompti 3/3 z vsebino) — veriga obravnava pošteno, na Renderu brez sekundarnega pomeni fallback → utemeljuje GEMINI_API_KEY na Render kot sekundarnega; tsc 0 src, eslint 0, 145/145, lokalna E2E 2× pravi načrti skozi novo pot; pred tem 1.48.1 — llms.txt/llms-full.txt GEO ozaveščenost dvojezičnega zemljevida (kandidat iz workloga 1.47): SL vrstica Zemljevid obogatena z iskrenim opisom 8 POI kategorij (vir OpenStreetMap) + števcem destinacij iz DESTINATIONS.length (ne hardcode), nova EN vrstica "Map — English" tik za njo (dvojezični par, EN imena kategorij), zgornji povzetek omenja "interaktivni zemljevid s točkami zanimivosti (OpenStreetMap)" + zemljevid dodan v EN trditev (jedro lijaka, zemljevid, krožni in jadranski vodniki /en); llms-full.txt: navodila agentu pošteno priznajo EN različice (jedro strani, zemljevid, cestni vodniki — prej samo "slovenščina") + obe zemljevidni vrstici z OSM opisi; verifikacija: curl diff obeh route (llms.txt +456 B, llms-full.txt +212 B — samo pričakovane vrstice), tsc 0 src, eslint 0, 145/145; opomba: dev v peskovniku zdaj `setsid --fork` (sirota pri PID 1 — preživi med sejami); pred tem 1.48.0 — EN PREVOD /zemljevid: zadnja SL-only površina v jedru produkta zdaj dvojezična — gumb "Map" v /en navigaciji je prej vodil na 308-nazaj na slovensko stran, zemljevid pa je za tuje turiste najbolj uporabna površina; jedro: ena vrstica na centralni EN whitelisti (EN_STATIC_ROUTES) samodejno aktivira proxy 308-guard + jezikovni switcher (preklop OHRANI stran) + sitemap EN URL s hreflang alternati + hreflang na strani; map-view (~940 vrstic Leaflet): vsi nizi v T-objektu (čipi 8 kategorij, gumbi, prazno stanje, error, info vrstica, aria, popupi destinacij/poti/POI), lang v deps Effectov ki bindajo Leaflet template stringe; popupi destinacij uporabijo obstoječi DESTINATIONS_EN overlay (tagline/duration z istim fallback vzorcem kot destinacijske strani — NIČ novih podatkovnih prevodov); poi-modal: CATEGORY_META {sl,en} + vsi modal nizi (About/Read more on Wikipedia/Contact & information/Phone/Website/Opening hours/Cuisine/Data:); stran: generateMetadata z locale-zavednim canonical + hreflangForPath (sl-SI/en-US/x-default); getEnSitemapUrlCount trdo kodiranih 11 → EN_STATIC_ROUTES.size (sitemap števec samo-vzdržen — test je padel točno na stari številki, zdaj nemogoče); E2E EN: nič 308, čipi/popupi/modal/switcher-preserved-path vse potrjeno, SL regresija čista, VLM desktop 9/10 mobilno 8/10, tsc 0, eslint 0, 145/145; ujeti med razvojem: kolizija L (Leaflet) vs L (prevodi) → T; prekrivanje network mockov **/api/pois** vs **/api/pois/osm-1** → ne-prekrivajoči vzorci; pred tem 1.47.0 — ZEMLJEVID ČIPI: uskladitev POI filtra /zemljevid s čip vzorcem klepeta iz 1.46 — prej enojni Select s 5/8 kategorij (hrana/nastanitve/trgovine skrite pred uporabniki, vsak preklop = nov Overpass klic); zdaj multi-select čipi z iskrenimi števci: 8 kategorij, prvi vklop plaste = EN klic category=all za 5 privzetih, izklop kategorije = skrivanje (0 klicev), cache preživi izklop/vklop plaste (instant), lazy upgrade posamičnega seznama ob eni aktivni kategoriji, prazno stanje + »Prikaži privzeto« iz cache, info vrstica »N POI · OSM« (iskrena oznaka vira), telemetrija map_poi_filtered (komplement chat_geo_filtered); E2E z network route mocki (Overpass v peskovniku padel — napakova pot iskreno potrjena) + ujeta napaka gnezdenja panela v kontrolni stolpec med E2E; VLM desktop 9/10, mobilno 8/10, 390 px 0 px preliva; pred tem 1.46.0 — KATEGORIJA ČIPI: Mindtrip raziskava (Wayback 2026-09-13 + iOS screenshoti VLM ×2 + aitravel.tools web screenshoti + 38 recenzij) → tripartitna odločitev → multi-select čipi kategorij s števci v GEO odgovorih klepeta IN fullscreen overlayju (izklop posodobi seznam in pine sočasno, iskren X/Y števec, prazno stanje s ponastavitvijo, overlay deduje filter + dobi legendo porekla, ki ji je manjkala); nova kategorija "destination" za T1 pini (prej kozmetični "stay" — filter "nastanitev" ne pokaže Bleda kot hotela; 2 h obiska v CATEGORY_DEFAULTS; nazaj združljivo s staro persistirano zgodovino); telemetrija cat_counts + nov dogodek chat_geo_filtered; ŽIVA PREVERBA AI POTI S CITATI (odložena od 1.42 — 429 val prešel): z-ai živo, citat [1,4] v odgovoru, 3 turkizni T2 pini z URL-ji slovenia.info; E2E SL+EN + persistenca round-trip + mobilno 390px 0px preliva + VLM 9/10; pred tem 1.45.0 — T2 SVEŽINA: trojna arhitektura svežosti uradnih virov STO — baseline (git verzioniran snapshot, uredniška kontrola) + runtime OVERLAY (freshness.ts: vzporeden prenos 3 llms.txt datotek, sanity gate ≥ max(100, 50 % baseline) in vse 3 datoteke OK — delni/boleči prenosi se zavrnejo, strežemo prejšnjo generacijo; lazy fire-and-forget na /api/chat in /api/ai/sources z 7-dnevnim TTL NE blokira odgovora) + TEDENSKI CRON /api/cron/sto-reingest (vercel.json torek 07:30 UTC, verifyCronAuth fail-closed) z raportom ODMIKA od baseline (dodani/odstranjeni viri + primeri — uredniški signal za ingest → git diff → commit); deljen parser sto-llms.ts za oba potoka (ekvivalenca 664/664, identični id-ji, odmik +0/−0 istega dne); /api/ai/sources razkriva novo polje source (baseline|overlay); enotsko 19/19 + E2E HTTP z živo dokazano varovalko (omrežni val 1/3 datotek → rejected-sanity, baseline nedotaknjen; potem 3× uspešna namestitev); z-ai 429 val od 1.42 dalje — živa preverba AI poti s citati ostaja odložena; pred tem 1.44.0 — T2 → PIN: citani uradni viri STO kot turkizni zaobljeni kvadrati na mini zemljevidu klepeta — zaključek triplastne arhitekture (zelena T1 preverjeno / jantarna OSM skupnost / turkizna STO uradno); zemljevid odseva ODGOVOR: pin dobi samo vir, ki ga AI dejansko citira ([n] v besedilu, deterministično parsanje 0 žetonov, fallback brez citatov = brez pinov — zemljevid ne laže); determinističen odmik 180–350 m od destinacije (FNV-1a hash, stabilen med sejami — članek je O kraju, pin sedi ob njem); čista stoHitToPlace + nova kategorija "source" (nikoli iz uporabnikovega besedila); članek ≠ postanek v treh plasteh obrambe (UI brez "+", addChatPlaceToItinerary zavrne "not-a-stop", stash preskoči); isValidChatPlace sprejme t2 (celotno sporočilo ne izpade ob reloadu); OSM budget 14→12 ob T2 pinih; telemetrija chat_geo_answered +t2_count; E2E SL+EN + persistenca round-trip + mobilno 390px 0px preliva verificirano (z-ai 429 val — AI pot s citati enotsko potrjena, živa preverba čaka okno, kot v 1.42/1.43); pred tem 1.43.0 — GEO → NAČRT, simetrija: EN KLIK ZA ODSTRANITEV klepet postanka — gumb »Odstrani« (X ikona, destruktiven šele ob hoverju, aria z imenom kraja) na kartici postanka z značko »Iz klepeta«; velja SAMO za klepet postanke (uporabnikova eksplicitna intencija — AI generirani ostanejo pod »Spremeni načrt«), poštena invalidacija F16 vzorca (quality/geoValidation/legs/routeGeometry se umaknejo in preračunajo na mestu uporabe), telemetrija `chat_place_removed` kot komplement `chat_place_added` (razmerje doda/odstrani = neposredna metrika kakovosti AI priporočil); pred tem 1.42.0 — GEO → NAČRT: »Dodaj v načrt« iz AI klepeta (Mindtripov »+«): vsak kraj iz AI odgovora (T1 destinacija ali OSM gostilna) ima gumb »+«, ki ga doda kot postanek v načrt — pametna izbira dneva (najbližji obstoječim postankom), časovni okvir za zadnjim postankom brez prekrivanj, OSM kraj prinese lastne koordinate → pin na zemljevidu poti + povezovalnik ~km/~min + opomba s poreklom in razkrita ocena stroška; brez načrta se kraj odloži v sessionStorage in samodejno doda ob prvi generaciji; telemetrija chat_place_added; pred tem 1.41.0 — GEO-ODGOVORI: AI klepet zdaj odgovarja na vprašanja »kje je hrana / pijača / tržnica« z MINI ZEMLJEVIDOM V KLEPETU (živi OSM kraji okoli destinacije + zeleni T1 pini iz odgovora AI, oštevilčeni markerji, značke porekla Preverjeno/OSM — zemljevid, ki prizna vir podatka, fullscreen overlay za mobilni, Overpass retry+mirror s 10-min predpomnjenjem in časovnim proračunom, lazy Leaflet ~0 KB za ostale strani, telemetrija chat_geo_answered; Mindtripov "generative spatial" vzorec v naši izvedbi; pred tem 1.40.0 — OPCIJA-3: rezervacija kot prvorazredni državljan načrtovalnika (CTA v statusnem traku/glavi dneva/čip na postankih, telemetrija booking_cta_clicked); pred tem 1.39.0 — DATA-LAYERS-RAG + duša: T2 plast 664 uradnih virov STO z citati [n] v AI odgovorih + jantarni hero; pred tem 1.38.0 — OPP-1: izkoriščanje okna priložnosti po padcu Mindtripovega weba (17. 9. 2026) — nova stran /primerjava (SL + /en/primerjava): iskrena uredniška primerjava specialista za Slovenijo s splošnimi AI načrtovalci (prizna njihove prednosti + lastno vrstico poraza "izven Slovenije"), primerjalna tabela, FAQPage JSON-LD, hreflang, sitemap/llms.txt/footer integracija, outreach odsek §8 z varovali znamke; cilja dolg rep "ai trip planner no signup" / "mindtrip alternative slovenia"; VLM desktop 8.5/10, mobilno 9/10; pred tem 1.37.0 — UX-CMP: implementacija vseh 6 popravkov iz UI/UX primerjave z Mindtripom — demo predogled dneva v praznem stanju načrtovalnika z gumbom »Poskusi ta primer« (VLM 8/10, »embarrassingly empty« razrešeno), odstranjen toast ob generiranju (prekrival je svež načrt), StopInsights collapse-by-default s povzetkom razlage, hub zemljevida s statistiko iz dataseta (22 destinacij · 9 regij · ocena 4,5) + čip legendo + odstranjeno duplikatno glavo, primarna paleta v globlji emerald (oklch 0.43 0.105 158), mobilni chat FAB scroll-aware; 1.36.3 — VERCEL-DEMO-PAY: `DSA_DEMO_PAYMENTS=1` nastavljena TUDI na Vercelu prek `vercel-env-set.sh --sync` (edina preostala dashboard točka iz 1.36.2 zaključena) — demo rezervacije zdaj delujejo na OBEH produkcijah (before/after dokaz: 501 → 200 confirmed); 1.36.2 — PROD-DEPLOY: Neon migracija `20260916100000_restrict_money_fks` uveljavljena (FK RESTRICT dokazan v pg_constraint), `DSA_DEMO_PAYMENTS=1` na Render — rezervacije spet delujejo, `render-env-set.sh` prevezan na novi Render `/v1` API + `--sync` res sproži deploy; 1.36.0 — REVIZIJA-10: adversarial audit celotnega poslovnega toka — 1×P1 varovalka listing-DELETE (Sponsorship kaskada bi uničila finančno evidenco) + 15×P2: demo plačila fail-closed (`DSA_DEMO_PAYMENTS` izrecni privolitev v produkciji), pogojni statusni prehodi rezervacij (409), sponsorship/consultation TOCTOU zaklope, accountType guardi na plačilnih rutah, AI-trust wrapProviderData+SYSTEM_DATA_GUARD na vseh DB→prompt poteh, provizijski webhook pogojni mark-paid z detekcijo dvojnega plačila, FK RESTRICT migracija za denarne zapise; 145/145 testov; 1.36.1 — DEPLOY-MIGR: ops skripta `scripts/ops/migrate-deploy.sh` za varni `prisma migrate deploy` na Neon tudi iz klona z lokalno sqlite shemo); pred tem 1.31.0 — CI-FUNC + PROD-MONITOR: prvi funkcionalni dim v CI — standalone zagon + 14 točk vključno fallback potjo načrtovalnika; samodejni health monitoring obeh produkcij vsake 3 ure z e-poštnim alarmom ob degraded/SEO regresiji; živo dokazano 14/14 lokalno, 13/13 Vercel, 13/13 Render; pred tem 1.30.0 MIGR-AUTO-BASELINE: startup korak `migrate:baseline` sam zapiše migration zgodovino na produkciji ob prvem zagonu — brez ročnega ukaza in brez Neon URL-ja v CI (poverilnica živi samo v dashboardu); oblika vrstice eksperimentalno preverjena, dirkalno-varna, 133/133 testov; pred tem 1.29.0 I18N-EVENTS + OWNERSHIP-SAFE + GEO-TESTS: EN dogodki po vzorcu DESTINATIONS_EN na vseh 5 klicnih mestih + EN UI sekcije, requireOwnership opt-in allowStaffAccess, 32 novih GEO/i18n regresijskih testov — 126/126; pred tem SEC-HARDENING 1.28.0 timing-safe + login ip+email, CI-TESTS + MIGR-HISTORY + FAIL-MODE 1.27.0: testi v CI, prisma/migrations zgodovina z drift vrati, /api/health izpostavi startup migracije). CI ✅ (Build + Lint/TypeCheck + Test + Migration drift + **Functional smoke**). Kateri commit je v produkciji, preveriš s smoke skripto (točka 8) + `/api/health`; samodejno ga vsake 3 ure preverja tudi Production Health Monitor.
> - **Produkcija:** stanje preveri dinamično — `bash scripts/verify/production-smoke.sh` (P8/FW2 markerji, anti-enumeracija, cron fail-closed ×7, Vercel commit status). ✅ Kvota `api-deployments-free-per-day` je zadela 2026-09-17 zvečer (commit status `c4bfbaa`: "Deployment rate limited — retry in 24 hours") — a se je ROLLING okno (100/24 h) sprostilo že ~30 min kasneje: od `5df9983` dalje Vercel spet deploya (1.48.2 je pristal istočasno na OBEH produkcijah, `/api/health` ok na obeh; zlati poti verificirani živo — Vercel 82 s `source: ai` (full načrt), Render večernje vrste > 120 s → pošten fallback po VEZANI časovnici). CI ZNOVA ZELEN po 27 h tihega rdečega niza (16 commitov od `4c2faf1` — `config:secrets` guard 1.33.0 je upravičeno padal na prekratkih CI env skrivnostih; fix `08de512` = CI completed success ×2). Preverjeno 2026-09-17 tudi: `f5f162e` / 1.48.1 GEO ozaveščenost zemljevida živ na obeh: `/api/health` na OBEH produkcijah potrjuje verzijo, preverjeno 2026-09-17 (Vercel + Render: GEO ozaveščenost zemljevida živi v produkciji — llms.txt na obeh vsebuje obogateno SL vrstico Zemljevid (8 POI kategorij, vir OpenStreetMap, števec iz DESTINATIONS.length) + novo vrstico "Map — English" (/en/zemljevid), llms-full.txt posodobljena navodila agentu o EN različicah + obe zemljevidni vrstici z OSM opisi — vse z gostiteljsko-zavednimi absolutnimi URL-ji (resolveBaseUrl: vsak gostitelj svoje); homepage + `/en/zemljevid` 200 na obeh; homepage na Vercelu 200 po retryu — prvi HTTP 000 prehodna omrežna napaka peskovnika (isti vzorec kot 1.47/1.48), direkti retry 200); Render avtomatsko iz `main`); runbook za primer naslednjega okna: [spodaj](#deploy-po-rate-limit-okni-p9).
