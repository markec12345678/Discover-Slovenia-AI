...
> - **Produkcija:** stanje preveri dinamično — `bash scripts/verify/production-smoke.sh` (P8/FW2 markerji, anti-enumeracija, cron fail-closed ×7, Vercel commit status). ✅ Kvota `api-deployments-free-per-day` (zastala pri `8f419eb`, 2026-09-11) se je sprostila 2026-09-12 — vsi nadaljnji deployi zeleni (nazadnje `8224fd9` / 1.47.0 — `/api/health` na OBEH produkcijah potrjuje verzijo, preverjeno 2026-09-17 (Vercel + Render: čipi POI kategorij na /zemljevid živijo v produkciji — multi-select 8 kategorij, hrana/nastanitve/trgovine razkrite; homepage + /zemljevid 200 na obeh ×3 retry — prvi HTTP 000 prehodna omrežna napaka peskovnika, direkti retry 200); Render avtomatsko iz `main`); runbook za primer naslednjega okna: [spodaj](#deploy-po-rate-limit-okni-p9).
