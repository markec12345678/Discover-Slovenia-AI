...
> - **Produkcija:** stanje preveri dinamično — `bash scripts/verify/production-smoke.sh` (P8/FW2 markerji, anti-enumeracija, cron fail-closed ×7, Vercel commit status). ✅ Kvota `api-deployments-free-per-day` (zastala pri `8f419eb`, 2026-09-11) se je sprostila 2026-09-12 — vsi nadaljnji deployi zeleni (nazadnje `c32d33e` / 1.43.0 — `/api/health` na OBEH produkcijah potrjuje verzijo, preverjeno 2026-09-17; Vercel homepage 200 + vse startup točke ok, Render (po cold startu) enako; Render avtomatsko iz `main`); runbook za primer naslednjega okna: [spodaj](#deploy-po-rate-limit-okni-p9).
