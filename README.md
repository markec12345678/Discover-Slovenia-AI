...
> - **Produkcija:** stanje preveri dinamično — `bash scripts/verify/production-smoke.sh` (P8/FW2 markerji, anti-enumeracija, cron fail-closed ×7, Vercel commit status). ✅ Kvota `api-deployments-free-per-day` (zastala pri `8f419eb`, 2026-09-11) se je sprostila 2026-09-12 — vsi nadaljnji deployi zeleni (nazadnje `8ee8001` / 1.46.0 — `/api/health` na OBEH produkcijah potrjuje verzijo, preverjeno 2026-09-20 (Vercel + Render: kategorija čipi geo odgovorov živi v produkciji; homepage 200 na obeh, smoke 13 pass — edini fail prehoden HTTP 000 peskovnika, retry 200); Render avtomatsko iz `main`); runbook za primer naslednjega okna: [spodaj](#deploy-po-rate-limit-okni-p9).
