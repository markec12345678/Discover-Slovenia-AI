...
| Plačila | `STRIPE_*` | ne | brez ključev demo SAMO z `DSA_DEMO_PAYMENTS=1` (1.36.0 fail-closed: prej je pomotoma unset `STRIPE_SECRET_KEY` v produkciji tiho vklopil demo vejo — brezplačne nadgradnje/»paid« naročila; zdaj brez ključa in brez zastavice plačilni tokovi vračajo 503/501). **Dejansko stanje (1.36.2, 2026-09-17):** Render = `DSA_DEMO_PAYMENTS=1` AKTIVNA (demo rezervacije delujejo), Vercel = brez zastavice (plačila zaprta) |
