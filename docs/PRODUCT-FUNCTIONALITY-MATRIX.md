...
> **Datum:** 2026-09-26 · **Verzija:** 1.100.3 (audit) / 1.102.0 (T5-B) / 1.104.0 (T5-D) / 1.106.0 (TASK 28 Tier 1: live-sync + overjena rezervacija) / 1.107.0 (TASK 30 Tier 1 #4: Lighthouse + CWV vrata) / 1.108.0 (TASK 31 Tier 1 #3: uvoz rezervacij prek e-pošte — TripItov model) / 1.109.0 (TASK 32 Tier 1 #5: EN blog + čiščenje de/it) / 1.110.0 (TASK 33 Tier 2 #1: koledar razpoložljivosti — preprečitev overbookinga) / 1.111.0 (TASK 34 Tier 2 #2: payout ledger — mesečne poravnave ponudnika) / 1.111.1 (TASK 35: payout sweep števec olderOpenCount — UI semantika izdaje) / 1.112.0 (ISSUE #8 / TASK 36 Faza 1: kanonski Dodaj v mojo pot + Moja pot + tab vrstica + lupina) / 1.113.0 (ISSUE #8 / TASK 37 Faza 2: strežniška refleksija zbirke + fork poti + vodiči/konzultacije dodaj + Go Mode pomiritev) / 1.114.0 (ISSUE #8 / TASK 38 Faza 3: en načrtovalnik + drift popravljeni + družina stanj + Start Anywhere dvig + wishlist most) · **HEAD ob auditu:** a854404 · **Popravljeni v T5-B (1.102.0):** H1, H2, M3-chat, M4, M5, M6 · **Popravljeni v T5-C (1.103.0):** M11, M9-pot · **Popravljeni v T5-D (1.104.0):** M1, M7, M8, M9-arhiv, M10 (označeno spodaj)
...
| **Koledar razpoložljivosti izkušnje** | **DODANO v 1.110.0 (TASK 33 Tier 2 #1 — mandat COMPETITIVE-ANALYSIS C1)** — `ExperienceAvailability` (privzeta kapaciteta/dan + sezona, tudi čezletna) + `ExperienceAvailabilityDay` (blackout / izjemni dan, `@@unique(experienceId,date)`); **atomarna preprečitev overbookinga** v POST /api/bookings (preverba ZNOTRAJ SERIALIZABLE transakcije, zasedenost = Σ groupSize rezervacij dneva s statusom ≠ cancelled); javni GET mesečni pogled + lastniški urejevalnik z obsegi; DORMANT brez nastavitev (obstoječe obnašanje nespremenjeno); 51 testov (domena + migracija + source-contract + funkcionalno DB) |
| **Payout ledger / mesečne poravnave** | **DODANO v 1.111.0 (TASK 34 Tier 2 #2 — mandat COMPETITIVE-ANALYSIS B3 »računovodstvo ponudnika: kdor je dobil koliko«)** — `PayoutEntry` (knjigovodska postavka po rezervaciji: snapshot bruto/stopnja/provizija/neto; `bookingId @unique` = idempotentna sync; FW1: SAMO plačane nepreklicane) + `PayoutSettlement` (mesečna izjava, `@@unique(ownerId, periodStart)`; zajem »vse odprto do vključno meseca« = sweep pozneje plačanih starejših obdobij; anti-race settle v transakciji); `syncPayoutEntries` samo-zdravi evidenco ob vsakem ogledu; stopnja 0.12 UVOŽENA iz commissions (en vir resnice), premium = 0 % a poravnave VELJAJO; CSV računovodsko poročilo (`;`/BOM/vejica/ISO datumi); zavihek Izplačila; audit sled payout_settlement_generated/settled; BREZ prenosov denarja (B2 Stripe Connect prihodnji korak); 45 testov (domena + migracija + source-contract + funkcionalno DB); POPRAVLJENO v 1.111.1 (TASK 35): GET vrača `olderOpenCount` (odprte postavke STROGO pred poravnavanim mesecom; tekoči mesec izključen — pripada naslednji poravnavi), gumb izdaje + sweep hint zrcalita strežniški pogoj issuePayoutSettlement (brez lažnih overcountov) |
| **Discovery UX 2.0 — Faza 2 „Zbirka je račun + skupnost“** | **DODANO v 1.113.0 (ISSUE #8 / TASK 37 — ZERO FEATURE LOSS nadaljuje)** — **strežniška refleksija zbirke „Moja pot“**: Prisma `UserTripItem` (identiteta userId:kind:refId, FIFO 200, CASCADE; startup migracija + lineage `20260928100000`) + `/api/my-trip` (GET · POST union-merge NIKOLI destruktiven · DELETE eksplicitno; samo B2C seje; isti sanitizacijski kanon kot klient) + `src/lib/my-trip-sync.ts` (prijavni sync push-union+pull, prazna lokalna=GET-only, diff-sync s senco debounce 2,5 s) + nevidni `MyTripAccountSync` v Navigation (gost = 0 klicev) + prijava/registracija sync + pull na /moja-potovanja → **zbirka je vezana na račun — cross-device** (Google Maps „Want to go“); **fork skupnostne poti**: „Shrani kot svojo kopijo“ na /pot/[shareId] → lasten shareId + editToken (popolnoma ureljiva kopija v „Moja potovanja“; „Zaženi Na poti“/PDF/kolaboracija nespremenjeni); **„Dodaj v mojo pot“ na vodičih** (detail + 22 kartic seznama, overlay Link vzorec) **+ konzultacijskih destinacijah** (zasebna stran 100 % client-side); **Go Mode pomiritev** (§24 „calm and focused“: hero podnaslov 1 vrstica, GPS pod NEXT — NOW→NEXT→WHEN→HOW→CONTEXT; vsa zmožnost nespremenjena); +85 testov (3402), matrika 31/31 · 0 izgube |
| **Discovery UX 2.0 — Faza 3 „En načrtovalnik, umirjena stanja“** | **DODANO v 1.114.0 (ISSUE #8 / TASK 38 — ZERO FEATURE LOSS nadaljuje)** — **§43 NO PARALLEL APP (Option A+C)**: /potovanje okvirjen kot korak ponudnikov/logistike ENEGA načrtovalnika — vrstica odnosa v heroju + povezava „Odpri AI načrtovalnik“ (prva vsebinska povezava /potovanje→/nacrtuj) + tiha vrstica obrazca + /nacrtuj vedno-vidna povezava „Celotno potovanje“ + Sheet „Več“ dostop (prej nikjer v navigaciji) + noga preoblikovana + predlogi destinacij „Iz moje poti“ (čipi, klik = vidna izbira); **DRIFT A/B popravljeni (§40)**: zbirka „Moja pot“ = resnica ogledala izbir /potovanje — re-search REHIDRIRA izbire (brskalniško dokazano), odstranitev iz zbirke živo deselecta (cross-tab); dogodki session-only (meja ohranjena); čista `selection-mirror.ts`; **družina stanj (§31-33, P-STATE-2 zapor)**: `src/components/states/` (LoadingState role=status aria-live + EmptyState ikona/naslov/CTA ≥44px brez lastnih nizov + ErrorState role=alert + retry) — prevzem 10 površin (/potovanje skeleti kategorij, moja-potovanja, tržnica, lokali, shared-trip, wishlist prazno s CTA, events, destinacije, owner), 6/7 EmptyState klonov poenotenih, ~12 „Nalagam“ → SL/EN (F3-E priprava); **Start Anywhere dvig (§25, nikoli hero)**: „Začni s svojimi viri →“ vedno vidna (tudi zložena) + hashchange + noga + Sheet + USP vrstica + `trackIngestCompleted` merjenje; **wishlist most (zbirka-sloj, brez tihega AI)**: `wishlist-trip-bridge.ts` (identiteta enaka wishlist-sheet → dedup) + `useWishlist` + „Iz priljubljenih“ trak v MyTripView (čipi ×števec, isti prefill dogodek + quick-add + toast) + planner trak vključuje destinacije priljubljenih; +99 testov (3501), matrika 31/31+6 novih vrstic · 0 izgube |
| **Discovery UX 2.0 — Faza 1 „The Spine“** | **DODANO v 1.112.0 (ISSUE #8 / TASK 36 — UX arhitektura DISCOVER→EXPLORE→SAVE→ADD→PLAN→GO z ZERO FEATURE LOSS)** — živi benchmark 15 UX področij (Mindtrip/ALMA/Wanderlog + GMaps/Airbnb/Tripadvisor +9, 86 virov, `docs/audit/task8-benchmark-matrix.md`) + D8-B arhitektura + D8-A audit (66 zmožnosti); **kanonski „Dodaj v mojo pot“**: zbirka `dai:my-trip-items` (`src/lib/my-trip.ts`, dedup kind:refId, FIFO 200, cross-tab) + primitiva `<AddToTripButton>` (full/compact/icon ≥44px, SL/EN) + `useMyTrip`; write-through: zemljevid (izbira+zbirka), klepet (44px nadgradnja), journey, planner dogodki; nove površine: destinacijski modal/hub/things-to-do, SmartSearch (zaprta P-SEARCH-1), dogodki, lokali, doživetja, wishlist most; pogled „Moja pot“ na /moja-potovanja (grupiranje, Nadaljuj načrtovanje handoff) + trak „Iz moje poti“ v /nacrtuj (prefill SAMO prazne izbire, no silent AI) + mobilna tab vrstica (Razišči·Zemljevid·Načrtuj·Moja pot·Več) + enotna lupina na 17 straneh brez navigacije + hierarhija domače strani (13 blokov ohranjenih, validator v <details>) + akcijska vrstica 5→2+Več + hard 404 `/načrtuj` fix; regresijska matrika 31/31 zmožnosti — 0 izgube (`docs/audit/task8-feature-regression-matrix.md`); +120 testov |
