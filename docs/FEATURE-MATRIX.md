...
| Provider capability matrika (1.96.0 §5) | /vir-podatkov sekcija "Zmožnostna matrika rezervacijske plasti" (SL+EN, 17×11) | lib/supply/capability-matrix.ts (IZPELJANA iz registra/matrike/env Boolean) | LIVE samo z dejanskim živim odgovorom; quote 0 živih; booking USER_ATTESTED za affiliate; E2E stolpec | PLATFORMA (transparenca virov) | VISIBLE | NE |
| Deterministična personalizacija (1.96.0 §10) | engine=deterministic pot + refine hitri čipi | deterministic-itinerary.ts (budget-aware izbira + partyType boost + weekdayClosedIds) + refine det-prima | hitre akcije 0 LLM; interesi uporabnika > partyType; presežek proračuna iskren | PLAN | CONTEXTUAL | NE |
| Dokumenti poti (1.96.0 §15) | /pot kartica "Dokumenti poti" (dodaj/izbriši, značke vrst) | TripDocument model + /api/trip/[shareId]/documents + agregator documents | SAMO metapodatki + https povezava (binarna vsebina pri uporabniku); izvor vedno razkrit; brisanje samo avtor | MY TRIP | CONTEXTUAL | NE |
| Offline dnevna navigacija (1.96.0 §16) | /na-poti čipi dni + gumb Danes | go-view.ts dayOverride + offline.html V2 zapisi + matrika zmožnosti | ročna izbira pošteno označena; offline.html izriše AI itinererje (V2); ločene vrstice podatki/ploščice/navigacija/vreme/rezervacije | GO MODE | CONTEXTUAL | NE |
| Enotna svežina podatkov (1.97.0 §17+§19) | /vir-podatkov sekcija "Svežina podatkov" (6 vrstic + disclaimer posnetek≠živo) + stop-insights vrstica | lib/data-freshness.ts (čist modul: FRESH/STALE/UNKNOWN/LIVE + pragovi po razredu + SourceType LIVE/STATIC/USER/PROVIDER/GENERATED + formatDataAge sl/en) | FSQ snapshot 2025-02-06 kot KONSTANTA (3 hardcode kopice odpravljene); ura vedno injicirana; brez podatka → NEZNANO (nikoli ugibamo) | PLATFORMA | VISIBLE | NE |
| Razložljiva priporočila (1.97.0 §20) | product-modal + experience-modal "Zakaj: {why}" vrstica | ai-recommendations.ts (AI why izključno iz polj kandidata + deterministični fallback iz istih polj + whySource ai/deterministic) | why sme navajati SAMO podatke iz kandidata; izvor vrstice iskreno označen ("(iz podatkov)" kadar ne-AI); cache z why + legacy vrata | DISCOVER (marketplace) | CONTEXTUAL | NE |
| Trip versioning / undo (1.97.0 §22) | planner undo čip ("Razveljavi" — LIFO 10) + /pot "Zgodovina verzij" (leneco + Obnovi) | SavedItineraryRevision model + PATCH piše revizijo stare vsebine znotraj CAS (fail-open, retencija 20) + GET .../revisions (vrata ≥ EDITOR; seznam metapodatki, ?version=N vsebina) + updateItinerary PATCH-na-mesto pred novim shareId | "AI refinement ne sme nepreklicno prepisati tripa" — v1 → refinement → v2 z restore; obnovitev dela novo revizijo (sledenje celo); 409 iskren padec v novo povezavo | PLAN + MY TRIP | CONTEXTUAL | NE |
| Zamrznjen namerni vrstni red (1.98.0 §21) | gumb "Optimalno zaporedje" + namig 🔒 "Fiksni postanki ostanejo na svojih mestih" (samo kadar dan ima zamrznjene) | LocationVisit.intentLocked + lib/route-intent.ts (FIXED izbire + supply + booking_provider → strežnik obeh poti + refine ohrani po destination_id na 3 izhodih) + sanitize prenos | optimizacija NE uniči uporabnikovega namernega vrstnega reda — zamrznjeni na točno isti poziciji z lastnim terminom; izmišljene AI oznake odstranjene | PLAN | CONTEXTUAL | NE |
| Segmentna optimizacija dneva (1.98.0 §21) | optimizeDayOrder v2: prosti segmenti med zamrznjenimi (≤7 IZČRPNO/2-opt, povezovalniki do sosedov) | lib/route-order.ts + koordinate tudi iz visit.lat/lng (OSM/tuji kraji — Zagreb dela) | vsota lokalnih optimumov = optimalna celota med oglišči; termini prostih položajev urejeni (zmnožek ohranjen); hevristika ostaja VOZNA (55 km/h ×1,3 — nikoli trajekt/hoja) | PLAN | CONTEXTUAL | NE |
| Regresijska suita optimizacije (1.98.0 §21) | — (testna plast) | issue4-wave6-route-regression.test.ts (43 testov: 1/2/14 dni, duplikati, zaprt POI, manjkajoče/NaN koordinate, nemogoča ruta, prehod meje, trajekt resnica, hoja/vožnja osnova, namerni vrstni red, čistost, i18n pariteta) | celotna §21 matrika iz telesa Issue #4 pokrita; bestOrder pogodba za plan-check F13 nespremenjena | PLATFORMA | — | NE |
| Provenance destinacijske vsebine (1.99.0 §18) | hub /destinacija/[slug] sekcija "Vir vsebine" + modal provenance noga + /vir-podatkov §18 sekcija (statistika + tabela 9 uradnih virov + jezikovni model + NiST opomba) | lib/destination-provenance.ts (čist modul: registr 9 uradnih virov s curl-preverjenimi URL-ji + allowlista domen + resolver uradni↔uredniška kuracija + summary) | 38/38 destinacij nosi vir+datum: 9 uradnih (5 destinacijskih domen + 4 STO kanonične strani) + 29 ISKRENih uredniških kuracij (brez izmišljenih virov); NiST raziskava odkrito dokumentirana (portal ni preverljivo dosegljiv — ne kopiramo) | DESTINACIJE + /vir-podatkov | VISIBLE | NE |
| AS-OF resnica dataseta (1.99.0 §18) | "posodobljeno" vrstica (stop-insights + /vir-podatkov + hub/modal) | DESTINATIONS_DATA_AS_OF v destination-provenance.ts (preseljeno iz stop-insights, re-izvoz) + test past ≡ git log slovenia-data.ts | prejšnja konstanta "2026-09-13" je bila 7 dni NEREŠNIČNA (TASK 62 jo je prehitel) — zdaj "2026-09-20" z varovalko: vsebinska sprememba brez prenosa konstante pade test | PLATFORMA | VISIBLE | NE |
| Odpiralni čas na destinacijskih površinah (1.99.0 §18) | hub sekcija "Odpiralni čas: {note} (vir: domena)" + modal opening blok | DestinationOpening (F5.5) prikaz na 2 novih površinah (prej samo planner stop-insights) | uporabnik vidi urnike/zaprtja tam, kjer odloča o obisku; vir vedno prikazan | DESTINACIJE | VISIBLE | NE |
| addressCountry resnica JSON-LD (1.99.0 §18) | — (SEO/plast) | hub TouristDestination addressCountry = dest.country (prej hardkodirano "SI") | 16 HR/ME/AL destinacij (Zagreb, Kotor, Tirana …) ni več LAŽNO označenih "SI"; country je ISO 3166-1 alpha-2 (varovan testom) | DESTINACIJE | — | NE |
| EN overlay 38/38 + uredniška oznaka ocene (1.99.0 §18) | map popup EN po dest.id (bugfix 4 destinacije) + kartice /destinacije VIDEN "uredniška/editorial" ob ★ | slovenia-data-en pokritost 38/38 po ID (regresijski test) + homeDest.editorialShort i18n | ocena je vedno UREDNIŠKA — vidiči uporabniki zdaj vidijo kvalifikator (prej samo sr-only); EN zemljevid ne meša več SL besedila | DESTINACIJE + ZEMLJEVID | VISIBLE | NE |
