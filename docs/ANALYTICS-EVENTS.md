...
| `itinerary_audio_play` (1.80) | uspešen začetek predvajanja **zvočnega povzetka dneva** (TASK 89, gumb „Poslušaj“/„Listen“ v glavi dneva) | vsak uspešen začetek (napake se NE štejejo) | `day`, `lang` (`sl`/`en`), `surface` (`planner`/`shared`), `bytes` (velikost zvoka) | doseg TTS zmožnosti (vrzel do Mindtripa — audio itinerar); `surface=shared` pove, ali poslušajo tudi obiskovalci deljenih povezav (prijatelji brez računa); `bytes` posredna dolžina poslušanja |
